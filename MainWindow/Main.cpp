...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmdLine, INT nCmdShow)
{
	...
	int window_height = g_START_Y + 20 + (g_BUTTON_SIZE + g_INTERVAL) * 4 + g_INTERVAL * 2 + 55;
	...
	(
		...
		WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX, //dwStyle
...
INT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		HANDLE hIcon = (HICON)LoadImage(GetModuleHandle(NULL), sz_icon, IMAGE_ICON, 48, 48, LR_LOADFROMFILE);
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 3; j++)
			{
	...
	case WM_COMMAND:
	{
		...
		if (LOWORD(wParam) >= IDC_BUTTON_0 && LOWORD(wParam) <= IDC_BUTTON_9)
		{
			...
			sz_digit[0] = LOWORD(wParam) - 952;//получаем ASCII код нажатой цифры
